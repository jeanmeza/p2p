...
# DONE: implement the event queue!
...
class Simulation:
    ...
    def __init__(self):
        ...
        # DONE: set up self.events as an empty queue
        self.events = []

    def schedule(self, delay, event):
        """Add an event to the event queue after the required delay."""

        # DONE: add event to the queue at time self.t + delay
    ...
    def run(self, max_t=float("inf")):
        ...
        # DONE: as long as the event queue is not empty
        while len(self.events):
            # DONE: get the first event from the queue
